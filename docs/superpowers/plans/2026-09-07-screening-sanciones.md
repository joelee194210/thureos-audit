...
- Produces: `repository.ScreeningRepository{col}`, `repository.NewScreeningRepository(db) *ScreeningRepository`, `.Create(ctx, *models.ScreeningResult) error`, `.FindByRedFlagID(ctx, redFlagID primitive.ObjectID) ([]models.ScreeningResult, error)`, `.FindByID(ctx, id primitive.ObjectID) (*models.ScreeningResult, error)`, `.Dismiss(ctx, id primitive.ObjectID, status models.ScreeningStatus, reviewedBy primitive.ObjectID, notes string) error`, `.FindExpiredWhitelist(ctx) ([]models.ScreeningResult, error)`, `.UpdateAfterReeval(ctx, id primitive.ObjectID, outcome ScreeningOutcome, matches []models.NormalizedMatch) error`
