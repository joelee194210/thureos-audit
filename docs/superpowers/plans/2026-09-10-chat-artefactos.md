...
**Task 15 completo.** Los nueve pasos corridos: siete ya los cubrían pruebas escritas
durante la implementación, y los ocho primeros se caminaron además contra la pila viva
(Mongo 7 + Redis + backend en local, datos ingeridos por el endpoint real de carga y
artefactos generados por el chat, no fabricados).

| Paso | Resultado | Evidencia |
|---|---|---|
| 2 · Artefacto viejo | ✅ | `/run` sobre uno sin `sources` → 400 «este artefacto no se puede actualizar» (el motivo que la interfaz muestra junto al botón deshabilitado). Exporta igual, con su instantánea, y la fecha dice «sin corrida registrada» en vez de un año 1 falso |
| 3 · Ciclo completo | ✅ | Guardado con nombre, aparece en la biblioteca, y al abrirlo re-ejecuta y devuelve `ranAt` |
| 4 · Datos frescos | ✅ | **Panamá 300 → 800, Colombia 50 → 75, y apareció México con 1000.** Una categoría nueva no puede salir de una caché: el artefacto re-consulta de verdad |
| 5 · Multi-fuente | ✅ | Dos monitores → `series: ["Step4 Frescura", "Step5 Sucursal Norte"]`, pivotado a una fila por país con una barra de cada monitor. Sin x repetidas |
| 6 · Exportaciones | ✅ | XLSX real de 6.352 bytes. Cabeceras `País` / `Monto total`, no `_id`/`aggValue`. Los montos son celdas **numéricas**: se suman con fórmula |
| 7 · Exportar no muta | ✅ | `ran_at` leído en Mongo antes y después de bajar el XLSX: idéntico al milisegundo |
| 8 · Autorización | ✅ | Usuario B pidiendo `/run` y `/export.xlsx` de un artefacto de A → 404, **el mismo código** que un id inexistente. La biblioteca de B sale vacía |
| 9 · Suite completa | ✅ | eslint 0, tsc limpio, 127/127 frontend, 342 casos Go, `golangci-lint` 0, `gofmt` limpio, build verde, CI en verde |

**El paso 4 era el que el plan se puso a sí mismo como crítico** —«si no cambian, el
artefacto está sirviendo la caché y hay un bug»—. No los sirve: cambian.

**Paso 7, la nota estructural que sigue valiendo.** En todo el backend hay un solo
escritor de `cached_data`/`ran_at` (`SetCache`, en `chat_artifact_repo.go`) y un solo
llamador (`artifact_handler.go:165`, dentro de `Run`). `ExportXLSX` no escribe en
ninguna de sus tres ramas. La medición de arriba confirma lo que ya decía el código.

**Pruebas unitarias que cubren estos mismos pasos**, para quien toque el código y
quiera el ciclo corto en vez de levantar la pila: paso 2 →
`TestRunArtifact_SinSourcesNoEjecuta`, `TestSnapshotData_*`; paso 5 →
`TestShape_ChartMultiFuenteDevuelveLosLabelsComoSeries`,
`TestShape_TablaMultiFuenteConcatena`; paso 6 →
`TestBuildArtifactXLSX_LosNumerosSonNumeros`, `TestBuildArtifactXLSX_UsaLosRotulosComoCabecera`,
`TestBuildArtifactXLSX_InstantaneaSinCorridaNoImprimeElAno1`; paso 8 →
`TestRunArtifact_DeOtroUsuarioEsNotFound`.
