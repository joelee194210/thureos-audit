...
- **Monitores de tipo `json`, tanto validación de estructura como por fila.** `ingestDelimited` (CSV/TXT) e `IngestExcel` comparten la misma forma de fila (`[]string`) y la misma forma de detectar su schema (`detectSchemaFromCSV`) — la validación de estructura y por fila se implementa una sola vez y cubre a los tres. `IngestJSON` usa un parser y una detección de schema distintos (`detectSchemaFromJSON`, valores ya tipados por `encoding/json`) — extenderlo con el mismo mecanismo de comparación/rechazo requeriría su propio camino de detección-antes-de-escribir, no solo su propio validador de filas. Se deja COMPLETO fuera de esta iteración (ni rechazo de estructura ni validación de fila para JSON) para no dejar una mitad de la validación implementada sin la otra — se agrega en una iteración aparte si hace falta. Los uploads a monitores JSON siguen funcionando exactamente igual que hoy (sin ningún gate nuevo).
